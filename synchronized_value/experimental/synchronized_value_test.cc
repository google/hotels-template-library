...
#include "third_party/hotels_template_library/synchronized_value/experimental/synchronized_value.h"
...
#include "third_party/hotels_template_library/synchronized_value/mutex_strategy.h"
#include "third_party/hotels_template_library/synchronized_value/rw_mutex_strategy.h"
...
void update_map(T& sv, int key, Updater&& updater) {
  if constexpr (Inplace)
    sv.update_inplace([key, &updater](std::absl::node_hash_map<int, int>& map) {
      updater(map[key]);
    });
  else
    sv.update_copy(
        [key, &updater](const std::absl::node_hash_map<int, int>& map) {
          auto new_map(map);
          std::this_thread::yield();
          updater(new_map[key]);
          return new_map;
